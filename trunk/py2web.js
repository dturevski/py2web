...
}N.push(0);ab.push(0);W=m(ac);while(true){ae=132;for(var Z=0;Z<R[N[N.length-1]].length;
...
if(J){b()}}if(ae==132){if(A){D("Error detected: There is no reduce or shift on the symbol "+ad[W])
...
for(var Z=0;Z<N.length;Z++){Q[Z]=N[Z];ah[Z]=ab[Z]}while(ae==132&&W!=61){if(A){D("\tError recovery\nCurrent lookahead: "+ad[W]+" ("+ac.att+")\nAction: "+ae+"\n\n")
}if(W==-1){ac.offset++}while(ae==132&&N.length>0){N.pop();ab.pop();if(N.length==0){break
}ae=132;for(var Z=0;Z<R[N[N.length-1]].length;Z+=2){if(R[N[N.length-1]][Z]==W){ae=R[N[N.length-1]][Z+1];
break}}}if(ae!=132){break}for(var Z=0;Z<Q.length;Z++){N.push(Q[Z]);ab.push(ah[Z])
}W=m(ac)}if(ae==132){if(A){D("\tError recovery failed, terminating parse process...")
...
break;case 37:ab[ab.length-8].rebirths.push({unit:ab[ab.length-5],at:ab[ab.length-4],prom:ab[ab.length-2]});
ai=ab[ab.length-8];break;case 38:ab[ab.length-6].rebirths.push({unit:ab[ab.length-3],at:ab[ab.length-2],prom:null});
ai=ab[ab.length-6];break;case 39:ab[ab.length-9].antirebirths.push({unit:ab[ab.length-7],from:ab[ab.length-6],to:ab[ab.length-4],prom:ab[ab.length-2]});
ai=ab[ab.length-9];break;case 40:ab[ab.length-7].antirebirths.push({unit:ab[ab.length-5],from:ab[ab.length-4],to:ab[ab.length-2],prom:null});
ai=ab[ab.length-7];break;case 41:ab[ab.length-6].promotions.push({unit:ab[ab.length-2],at:ab[ab.length-4]});
ai=ab[ab.length-6];break;case 42:ab[ab.length-6].recolorings[ab[ab.length-2]].push(ab[ab.length-4]);
ai=ab[ab.length-6];break;case 43:ab[ab.length-4].imitators=ab[ab.length-2];ai=ab[ab.length-4];
break;case 44:ai=ab[ab.length-2].setv("checksign",ab[ab.length-1]);break;case 45:ai=ab[ab.length-1];
break;case 46:ai=ab[ab.length-2].setv("annotation",ab[ab.length-1]);break;case 47:ai=ab[ab.length-2].setv("comments",ab[ab.length-1]);
break;case 48:ai=ab[ab.length-1].setv("promotion",ab[ab.length-2]).setv("departant",ab[ab.length-2]);
break;case 49:ai=ab[ab.length-1];break;case 50:ai=ab[ab.length-1];break;case 51:ai=new r(ab[ab.length-1],ab[ab.length-2],"");
break;case 52:ai=new r(ab[ab.length-1],ab[ab.length-3],ab[ab.length-2]);break;case 53:ai=ab[ab.length-1];
break;case 54:ai=ab[ab.length-1];break;case 55:ai=ab[ab.length-1];break;case 56:ai=new r(ab[ab.length-1],"u","");
break;case 57:ai=new r(ab[ab.length-1],ab[ab.length-2],"");break;case 58:ai=new r(ab[ab.length-1],ab[ab.length-3],ab[ab.length-2]);
break;case 59:ai=new r(ab[ab.length-1],"u",ab[ab.length-2]);break;case 60:ai=ab[ab.length-1].setv("promotion",new r("P","u","")).setv("departant",new r("P","u",""));
break;case 61:ai=ab[ab.length-2].setEnPassant();break;case 62:ai=new v(ab[ab.length-3],ab[ab.length-1],-1);
break;case 63:ai=new v(ab[ab.length-3],ab[ab.length-1],ab[ab.length-1]);break;case 64:ai=new v(ab[ab.length-5],ab[ab.length-1],ab[ab.length-3]);
break;case 65:ai=[ab[ab.length-1]];break;case 66:ab[ab.length-3].push(ab[ab.length-1]);
ai=ab[ab.length-3];break;case 67:ab[ab.length-2].push(ab[ab.length-1]);ai=ab[ab.length-2];
break;case 68:ai=new n(true);break;case 69:ai=new n(false);break;case 70:ai=ab[ab.length-1];
break;case 71:ai=ab[ab.length-1];break;case 72:ai=[ab[ab.length-1]];break;case 73:ab[ab.length-2].push(ab[ab.length-1]);
...
}else{P="x"+w(this.capture)+"&rarr;"+w(this.arrival)}}var O=R+Q+P;if(this.departant.asText()!=this.promotion.asText()){O+="="+this.promotion.asText()
}O+=this.imitatorsAsText()+this.recoloringsAsText()+this.antirebirthsAsText()+this.rebirthsAsText()+this.promotionsAsText();
